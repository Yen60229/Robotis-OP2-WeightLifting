...
void ColorFinder::Filtering(Image *img)
{
    ...
    h_max = m_hue + m_hue_tolerance+20;
    h_min = m_hue - m_hue_tolerance-20;
    if(h_max > 360)
        h_max -= 360;
    if(h_min < 0)
        h_min += 360;
...
Point2D& ColorFinder::RGBGetPosition(Image* img)
{
    int sum_x = 0, sum_y = 0, count = 0;
    ...
    if(count <= (img->m_NumberOfPixels * m_min_percent / 100) || count > (img->m_NumberOfPixels * m_max_percent / 100))
    {
        m_center_point.X = -1.0;
        m_center_point.Y = -1.0;
    }
...
void ColorFinder::RGBFiltering(Image *img)
{
    unsigned int R, G, B;
    ...
    for(int i = 0; i < img->m_NumberOfPixels; i++)
    {
        R =  img->m_ImageData[i*img->m_PixelSize + 0];
        G =  img->m_ImageData[i*img->m_PixelSize + 1];
        B =  img->m_ImageData[i*img->m_PixelSize + 2];

	    


        if( ((int)(R-G)>=100) && ((int)(R-B)>=100) )
        {
                    m_result->m_ImageData[i]= 1;
	}
        else
