...
BallFollower2::BallFollower2()
{
	...
	m_KickTopAngle = 0.0;
	m_KickRightAngle = -30.0;//-30
	m_KickLeftAngle = 10.0;//30
...
void BallFollower2::Process(Point2D ball_pos)
{
	if(DEBUG_PRINT == true)
		fprintf(stderr, "\r                                                                               \r");
	...
	{
		if(Walking2::GetInstance()->IsRunning() == true)
			Walking2::GetInstance()->Stop();
		else
		{
			if(m_KickBallCount < m_KickBallMaxCount)
				m_KickBallCount++;
	...
	else
	{
		...
		else
		{
			...
			Walking2::GetInstance()->X_MOVE_AMPLITUDE = m_FBStep;

			if(m_RLTurn < m_GoalRLTurn)
				m_RLTurn += m_UnitRLTurn;
			else if(m_RLTurn > m_GoalRLTurn)
				m_RLTurn -= m_UnitRLTurn;
			Walking2::GetInstance()->A_MOVE_AMPLITUDE = m_RLTurn;
